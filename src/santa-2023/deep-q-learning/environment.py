...
class Environment:
    def __init__(self, grid_size, fitness_max, allowed_moves, num_wildcards, render_on=False):
        self.grid_size = grid_size

        self.move_count = len(allowed_moves)
    ...
    def add_agent(self):
        # Choose a random value of last move, but compute current fitness
    ...
    def add_goal(self):
        """grid is 2-dimensional, last_move x fitness"""
        locations = []
        for i in range(len(self.allowed_moves)):
            for j in range(self.num_wildcards + 1):
                fitness = self.fitness_max - j - 1
                locations.append((i, fitness))
                self.grid[i][fitness] = -1
