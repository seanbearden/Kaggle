...
class Group:
    ...
    def trace_generators(self):
        ...
        """
        if self.stab is None:
            return self.gens_trace
        else:
            return self.stab.trace_generators() | self.gens_trace
    ...
    def add_gen(self, gen, trace=None):
        """Add a generator to this group.
        """
        # print(f'add_gen() called for basepoint {self.basepoint}')
        # print(f'for generator {gen}')
        gen, gen_trace = self.sift_gen(gen, trace=trace)
        ...
        if not is_id_perm(gen):
            ...
            self.add_nonmember_gen(gen, trace=gen_trace)
    ...
    def monte_carlo_round(self):
        ...
        p = self.rng.sample()
        p, p_trace = self.sift_gen(p, trace=None)
        p_is_missing = not is_id_perm(p)
        if p_is_missing:
            self.add_nonmember_gen(p)
        return p_is_missing

    def sift_gen(self, gen, trace=None):
        """Perform sifting for adding a new generator.
        """
        if self.cfg.deep_sift_gens:
            return self.deep_sift(gen, trace=trace)
        else:
            return self.sift(gen, trace=trace)


    def sift(self, p, depth=math.inf, trace=None):
        """Perform sifting on p.

        Args:
            p (list): The permutation to sift.
            trace (list): The trace list to record the steps (if used).
            depth (int): The depth to which sifting should occur.

        Returns:
            tuple: The sifted permutation, and the trace of applied generators.
        """
        ...
        try:
            if trace is None:
                ...
                p_stab, trace = self.move_to_basepoint(p[self.basepoint], p, trace)

        except NotInOrbit:
            return p, trace

        p_sifted, trace = self.stab.sift(p_stab, trace=trace, depth=depth - 1)

        return p_sifted, trace

    def deep_sift(self, p, trace=None):
        ...
        try:
            p_stab, p_stab_trace = self.move_to_basepoint(p[self.basepoint], p, trace=trace)
        except NotInOrbit:
            for a in sorted(self.tree.keys()):
                if p[a] in self.tree:
                    # different traces?
                    p_tmp, p_tmp_trace = self.move_to_basepoint(a, inv_perm(p), trace=trace)
                    p_tmp = inv_perm(p_tmp)
                    p_stab, p_stab_trace = self.move_to_basepoint(p_tmp[self.basepoint], p, trace=trace)
                    # p_stab = self.move_to_basepoint(p_tmp[self.basepoint], p)
                    return self.stab.deep_sift(p_stab, trace=p_stab_trace)
            return p

        return self.stab.deep_sift(p_stab, trace=trace)
    ...
    def add_nonmember_gen(self, gen, las_vegas=False, trace=None):
        """Add a generator that is not a member of this group yet.
        """
        gen, gen_trace = self.sift(gen, trace=trace)
        ...
        if not is_id_perm(gen):
            ...
            if gen[self.basepoint] == self.basepoint:
                ...
                self.stab.add_nonmember_gen(gen, las_vegas, trace=gen_trace)
            else:
                # TECH DEBT: Must know the generators to create this generator
                print(f'add {gen}')
                self.gens_trace[tuple(gen)] = gen_trace
    ...
    def add_all_schreier_gens(self):
        ...
        for gen, inv_gen in self.generators():
            for a in sorted(self.tree.keys()):
                ...
                # starts with inverse generator
                p, p_trace = self.move_to_basepoint(a, inv_gen, trace=[(inv_gen, 0)])
                # inverse of permutation
                p_trace.append((p, 2))
                p_inv = inv_perm(p)

                schreier_gen, gen_trace = self.move_to_basepoint(p_inv[self.basepoint], p_inv, trace=p_trace)

                self.stab.add_gen(schreier_gen, trace=gen_trace)
    ...
    def add_random_schreier_gens(self):
        ...
        for i in range(self.cfg.random_schreier_gens):
            ...
            p, p_trace = self.move_to_basepoint(a, inv_gen, trace=None)
            p = inv_perm(p)

            schreier_gen, schreier_gen_trace = self.move_to_basepoint(p[self.basepoint], p, trace=p_trace)

            # We can't use add_gen recursively for the monte carlo variant
            # so we manually sift and check for identity
            schreier_gen, schreier_gen_trace = self.sift_gen(schreier_gen, trace=schreier_gen_trace)
    ...
    def rebuild_schreier_tree(self):
        ...
        while True:
            ...
            # todo: how to use trace?
            trace = []
            new_gen, trace = self.move_to_basepoint(a, trace=trace)
    ...
    def remove_redundand_generators_step(self):
        ...
        while i < len(self.gens):
            ...
            if gen_i[self.basepoint] in self.tree:
                witness, witness_trace = self.sift(gen_i, trace=None)
    ...
    def insert_toplevel_block_stabilizer(self):
                ...
                for g in g_pair:
                    ...
                    for b in block_b:
                        if repr_map[b] not in (None, repr_b):
                            ...
                            witness, witness_trace = self.stab.sift(
                                mult_perm(inv_perm(part_b), part_c), trace=None)
        ...
        if len(blocks) == 1:
            residue, residue_trace = self.stab.sift(self.gens[0][0], trace=None)
    ...
    def change_base_conjugated(self, base, budget):
        ...
        if budget < 0:
            ...
            while group.order() < target_order:
                if gens:
                    ...
                    g = self.sample()
                self.cfg.stats.rounds += 1
                residue, residue_trace = group.sift(g, trace=None)
                if not is_id_perm(residue):
        ...
        while group.order() < target_order:
            if gens:
                ...
                g = self.sample(depth)
            self.cfg.stats.rounds += 1
            residue, residue_trace = group.sift(g, depth=sift_depth, trace=None)
            if not is_id_perm(residue):
                group.add_nonmember_gen(residue, las_vegas=True)
    ...
    def schreier_verify_step(self):
        ...
        for gen, inv_gen in self.generators():
            for a in sorted(self.tree.keys()):
                ...
                residue, residue_trace = self.stab.sift(schreier_gen, trace=None)
                if not is_id_perm(residue):
                    raise IncompleteStrongGeneratingSet(
                        "incomplete strong generating set detected"
    ...
    def simss_verify_step(self):
        ...
        for delta_i in deltas[1:]:
            ...
            for g, g_inv in stab.stab.generators():
                ...
                residue, residue_trace = stab.sift(g_u_delta_i_inv, trace=None)
        ...
        for gamma_ij in gammas:
            ...
            residue, residue_trace = stab.sift(b, trace=None)
            if not is_id_perm(residue):
                raise IncompleteStrongGeneratingSet(
                    "incomplete strong generating set detected"
                    " during Sims's verification step",
