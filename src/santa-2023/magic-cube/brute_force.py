...
def solve(ddf, puzzle_info, initial_state=None, debug=False):
    ...
    if num_wildcards < 2:
        ...
        while time.time() - START_TIME < RUN_TIME and time.time() - start_time < TIMEOUT:
            ...
            source_paths_str = {';'.join(map(str, k)) for k in source_paths.keys()}
            dest_paths_str = {';'.join(map(str, k)) for k in dest_paths.keys()}
